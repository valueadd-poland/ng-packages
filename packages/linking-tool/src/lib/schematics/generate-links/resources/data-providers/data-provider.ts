...
export class DataProvider {
  static getRouteDeclarations(source: SourceFile): RouteDeclaration[] {
    if (
      TypescriptApiUtil.isVariableDeclarationExisting(
        GenerateLinksProperty.Routes,
        source
      ) &&
      ...
      return TypescriptApiUtil.getArrayExpression(routesExpression)
        ...
        .map((expression) =>
          DataProvider.resolveRouteDeclaration(
  ...
  private static resolveChildren(
    ...
    return TypescriptApiUtil.getArrayValue(
      TypescriptApiUtil.getPropertyAssignment(
        GenerateLinksProperty.Children,
        object
      )
    ).map((expression) =>
      DataProvider.resolveRouteDeclaration(
  ...
  private static resolveLoadChildren(object: ObjectLiteralExpression): string {
    ...
    if (TypescriptApiUtil.isStringLiteral(loadChildrenAssignment)) {
      ...
      lazyLoadedModulePath = TypescriptApiUtil.getStringValue(
  ...
  private static resolvePath(object: ObjectLiteralExpression): string {
    return TypescriptApiUtil.getStringValue(
      TypescriptApiUtil.getPropertyAssignment(
        GenerateLinksProperty.Path,
        object
      )
    );
  }

  private static resolveRouteDeclaration(
    object: ObjectLiteralExpression
  ): RouteDeclaration {
    ...
    if (
      TypescriptApiUtil.isPropertyExisting(GenerateLinksProperty.Path, object)
    ) {
      result = {
        ...result,
