...
export class HostDirEntry implements DirEntry {
  ...
  private getSubfilesRecursively(): FileEntry[] {
...
export class HostTree implements Tree {
  ...
  static isHostTree(tree: Tree): tree is HostTree {
    ...
    if (
      typeof tree === "object" &&
      typeof (tree as HostTree)._ancestry === "object"
    ) {
      return true;
    }
  ...
  constructor(
    protected _backend: virtualFs.ReadonlyHost<any> = new virtualFs.Empty()
  ) {
    this._record = new virtualFs.CordHost(
      new virtualFs.SafeReadonlyHost(_backend)
    );
  ...
  branch(): Tree {
    ...
    branchedTree._recordSync = new virtualFs.SyncDelegateHost(
      branchedTree._record
    );
      ...
      switch (action.kind) {
        case "c": {
          ...
          if (
            ...
            this._record
              .overwrite(path, (content as any) as virtualFs.FileBuffer)
              .subscribe();
          } else {
            this._record
              .create(path, (content as any) as virtualFs.FileBuffer)
              .subscribe();
          }
        ...
        case "o": {
          ...
          this._record
            .write(path, (content as any) as virtualFs.FileBuffer)
            .subscribe();
  ...
  getDir(path: string): DirEntry {
    ...
    if (!maybeCache) {
      ...
      maybeCache = new HostDirEntry(
        parent && this.getDir(parent),
        p,
        this._recordSync,
        this
      );
...
export class FilterHostTree extends HostTree {
  constructor(
    tree: HostTree,
    filter: FilePredicate<boolean> = (): boolean => true
  ) {
    ...
    const recurse: (base: Path) => Observable<void> = (base) => {
      return originalBackend.list(base).pipe(
        ...
        concatMap((path) => {
          ...
          return newBackend.write(
            path,
            (content as any) as virtualFs.FileBuffer
          );
        })
      );
    };
