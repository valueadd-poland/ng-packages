...
describe("UpdateRecorderBase", () => {
  ...
  it("can create the proper recorder", () => {
    const e = new SimpleFileEntry(
      normalize("/some/path"),
      Buffer.from("hello")
    );
    expect(
      UpdateRecorderBase.createFromFileEntry(e) instanceof UpdateRecorderBase
    ).toBe(true);
    expect(
      UpdateRecorderBase.createFromFileEntry(e) instanceof UpdateRecorderBom
    ).toBe(false);
  });

  it("can create the proper recorder (bom)", () => {
    const eBom = new SimpleFileEntry(
      normalize("/some/path"),
      Buffer.from("\uFEFFhello")
    );
    expect(
      UpdateRecorderBase.createFromFileEntry(eBom) instanceof UpdateRecorderBase
    ).toBe(true);
    expect(
      UpdateRecorderBase.createFromFileEntry(eBom) instanceof UpdateRecorderBom
    ).toBe(true);
  });
