...
import { stripIndent } from "../../utils/runtime";
...
const representativeFile = readFileSync(
  require.resolve(__dirname + "/test/angular.json"),
  "utf8"
);
...
// @TODO tests don't pass when using jest
describe.skip("readJsonWorkpace Parsing", () => {
  ...
  it("parses a representative file", async () => {
    ...
    expect(Array.from(workspace.projects.keys())).toEqual([
      "my-app",
      "my-app-e2e",
    ]);
    ...
    expect(
      workspace.projects.get("my-app")!.targets.get("build")!.builder
    ).toBe("@angular-devkit/build-angular:browser");
  });

  it(`doesn't remove falsy values when using the spread operator`, async () => {
    const host = createTestHost(representativeFile);
    const workspace = await readJsonWorkspace("", host);
    const prodConfig = workspace.projects.get("my-app")!.targets.get("build")!
  ...
  it("parses extensions only into extensions object", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect(
      (workspace.projects.get("my-app") as any)["schematics"]
    ).toBeUndefined();
  });
...
describe("JSON WorkspaceDefinition Tracks Workspace Changes", () => {
  ...
  it("tracks moving and modifying an existing extension object", async () => {
    ...
    workspace.extensions["schematics"] = undefined;
    (workspace.extensions["schematics2"] as JsonObject)[
      "@angular/schematics:component"
    ] = {
      prefix: "xyz",
    };
    ...
    if (change) {
      expect(change.op).toBe("add");
      expect(change.value).toEqual({
        "@angular/schematics:component": { prefix: "xyz" },
      });
    }
  });
  ...
  it("tracks copying and modifying an existing extension object", async () => {
    ...
    change = metadata.findChangesForPath(
      "/schematics/@angular~1schematics:component"
    )[0];
    expect(change).not.toBeUndefined();
    if (change) {
      expect(change.op).toBe("replace");
...
describe("JSON ProjectDefinition Tracks Project Changes", () => {
  ...
  it("tracks property additions", async () => {
    ...
    const change = metadata.findChangesForPath(
      "/projects/my-app/sourceRoot"
    )[0];
    expect(change).not.toBeUndefined();
    if (change) {
      expect(change.op).toBe("add");
