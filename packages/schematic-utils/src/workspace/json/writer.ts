...
function convertJsonWorkspace(
  ...
  const obj = {
    ...
    projects: workspace.projects
      ? convertJsonProjectCollection(workspace.projects)
      : {},
  };
...
function convertJsonTarget(target: TargetDefinition): JsonObject {
  return {
    ...
    ...(isEmpty(target.options)
      ? {}
      : { options: target.options as JsonObject }),
...
function normalizeValue(
  ...
  switch (type) {
    ...
    case "projectcollection":
      const projects = convertJsonProjectCollection(
        value as Iterable<[string, ProjectDefinition]>
      );
    ...
    case "targetcollection":
      const targets = convertJsonTargetCollection(
        value as Iterable<[string, TargetDefinition]>
      );
  ...
  for (const [node, elements] of nodeChanges.entries()) {
    ...
    for (const element of optimizedElements) {
      if (typeof element === "string") {
        ...
        prefixComma =
          data.original[parentPoint - 1] !== "," ||
          removedCommas.has(parentPoint - 1);
      }
    }
  }
