...
export async function readJsonWorkspace(
  ...
  // Version check
  const versionNode = ast.properties.find(
  ...
  if (formatVersion !== 1) {
    throw new Error(
      `Invalid format version detected - Expected:[ 1 ] Found: [ ${formatVersion} ]`
    );
  }
...
const specialWorkspaceExtensions = [
  "cli",
  "defaultProject",
  "newProjectRoot",
  "schematics",
...
function parseWorkspace(
  ...
  for (const { key, value } of workspaceNode.properties) {
    ...
    if (name === "$schema" || name === "version") {
      ...
      if (
        !specialWorkspaceExtensions.includes(name) &&
        !/^[a-z]{1,3}-.*/.test(name)
      ) {
  ...
  let collectionListener:
    | DefinitionCollectionListener<ProjectDefinition>
    | undefined;
  ...
  const projectCollection = new ProjectDefinitionCollection(
    projects,
    collectionListener
  );
...
function parseProject(
  ...
  for (const { key, value } of projectNode.properties) {
    ...
    switch (name) {
      ...
      case "architect":
        if (value.kind !== "object") {
          context.error(
            `Invalid "${name}" field found; expected an object.`,
            value
          );
          break;
        }
      ...
      default:
        if (
          !specialProjectExtensions.includes(name) &&
          !/^[a-z]{1,3}-.*/.test(name)
        ) {
          context.warn(`Project extension with invalid name found.`, key);
        }
  ...
  let collectionListener:
    | DefinitionCollectionListener<TargetDefinition>
    | undefined;
  if (context.trackChanges) {
    if (targetsNode) {
