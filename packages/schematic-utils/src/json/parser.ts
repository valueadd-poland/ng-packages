...
export class InvalidJsonCharacterException extends JsonException {
  ...
  constructor(context: JsonParserContext) {
    ...
    super(
      `Invalid JSON character: ${invalidChar} at ${pos.line}:${pos.character}.`
    );
...
export class PathSpecificJsonException extends JsonException {
  constructor(public path: string, public exception: JsonException) {
    super(
      `An error happened at file path ${JSON.stringify(path)}: ${
        exception.message
      }`
    );
  }
...
function _readArray(
  ...
  while (_peek(context) != "]") {
    ...
    if (
      (context.mode & JsonParseMode.TrailingCommasAllowed) !== 0 &&
      _peek(context) === "]"
    ) {
      break;
    }
...
function _readIdentifier(
  ...
  const identValidFirstChar =
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ";
  const identValidChar =
    "_$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ0123456789";
  let first = true;
  let value = "";
  ...
  while (true) {
    ...
    if (
      char == undefined ||
      (first
        ? identValidFirstChar.indexOf(char)
        : identValidChar.indexOf(char)) == -1
    ) {
      context.position = context.previous;
    ...
    while (true) {
      ...
      if (char == "/" && context.original[context.position.offset + 1] == "*") {
        ...
        comments.push({
          ...
          text: context.original.substring(
            start.offset,
            context.position.offset
          ),
          content: context.original.substring(
            start.offset + 2,
            context.position.offset - 2
          ),
        });
      } else if (
        char == "/" &&
