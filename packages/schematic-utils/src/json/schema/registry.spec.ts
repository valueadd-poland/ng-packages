    ...
    registry
      ...
      .pipe(
        ...
        map((result) => {
          expect(result.success).toBe(false);
          expect(
            new SchemaValidationException(result.errors).message
          ).toContain(
    ...
    await registry
      ...
      .pipe(
        ...
        map((result) => {
          ...
          expect(result.errors && result.errors[0].instancePath).toBe(
            "/banana"
          );
          expect(result.errors && (result.errors[0].params as any).format).toBe(
            "is-hotdog"
          );
        })
      )
    ...
    registry
      ...
      .pipe(
        ...
        map((result) => {
          ...
          expect(deprecatedMessages[0]).toBe(
            'Option "foo" is deprecated: Use bar instead.'
          );
