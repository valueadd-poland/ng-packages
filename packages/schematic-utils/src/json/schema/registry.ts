...
import { deepCopy, PartiallyOrderedSet } from "../../utils/runtime";
...
export class SchemaValidationException extends BaseException {
  ...
  public static createMessages(errors?: SchemaValidatorError[]): string[] {
    ...
    const messages = errors.map((err) => {
      let message = `Data path ${JSON.stringify(err.instancePath)} ${
        err.message
      }`;
      if (err.params) {
        switch (err.keyword) {
          ...
          case "enum":
            message += `. Allowed values are: ${(err.params.allowedValues as
              | string[]
              | undefined)
              ?.map((v) => `"${v}"`)
              .join(", ")}`;
