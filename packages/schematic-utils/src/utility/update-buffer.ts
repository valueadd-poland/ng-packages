...
export class UpdateBuffer {
  ...
  protected _slice(start: number): [Chunk, Chunk] {
    ...
    const index =
      start >= this._originalContent.length
        ? start
        : this._getTextPosition(start);
  ...
  protected _getTextPosition(index: number): number {
    return Buffer.from(this._originalContent.toString().substring(0, index))
      .length;
  }
  ...
  toString(encoding = "utf-8"): string {
    return this._linkedList.reduce(
      (acc, chunk) => acc + chunk.toString(encoding),
      ""
    );
  }
  generate(): Buffer {
  ...
  insertLeft(index: number, content: Buffer, assert = false): void {
    this._slice(index)[0].append(content, assert);
  }
  insertRight(index: number, content: Buffer, assert = false): void {
