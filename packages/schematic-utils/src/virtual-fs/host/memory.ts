...
export class SimpleMemoryHost implements Host<{}> {
  ...
  private _watchers = new Map<
    Path,
    [HostWatchOptions, Subject<HostWatchEvent>][]
  >();
  ...
  protected _write(path: Path, content: FileBuffer): void {
    ...
    // Create the stats.
    const stats: Stats<SimpleMemoryHostStats> = this._newFileStats(
      content,
      old
    );
    ...
    this._updateWatchers(
      path,
      old ? HostWatchEventType.Changed : HostWatchEventType.Created
    );
  }
      ...
      for (const path of this._cache.keys()) {
        if (path.startsWith(from + NormalizedSep)) {
          ...
          if (content) {
            ...
            this._cache.set(
              join(to, NormalizedSep, path.slice(from.length)),
              content
            );
          }
        }
