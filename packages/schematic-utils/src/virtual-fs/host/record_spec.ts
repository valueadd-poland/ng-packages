...
describe("CordHost", () => {
  ...
  it("works (create)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "write", path: path`/blue` }]);
  ...
  it("works (create -> create)", (done) => {
    ...
    host.write(path`/blue`, fileBuffer`hi`).subscribe(undefined, done.fail);
    host
      .write(path`/blue`, fileBuffer`hi again`)
  ...
  it("works (create -> rename)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "write", path: path`/red` }]);
  ...
  it("works (create -> rename -> rename)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "write", path: path`/yellow` }]);
  ...
  it("works (rename)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "rename", from: path`/hello`, to: path`/blue` }]);
  ...
  it("works (rename -> rename)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "rename", from: path`/hello`, to: path`/red` }]);
  ...
  it("works (rename -> create)", (done) => {
    ...
    host.rename(path`/hello`, path`/blue`).subscribe(undefined, done.fail);
    host
      .write(path`/hello`, fileBuffer`beautiful world`)
      .subscribe(undefined, done.fail);
  ...
  it("works (overwrite)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "write", path: path`/hello` }]);
  ...
  it("works (overwrite -> delete)", (done) => {
    ...
    expect(
      target.records.filter((x) => mutatingTestRecord.includes(x.kind))
    ).toEqual([{ kind: "delete", path: path`/hello` }]);
  ...
  it("works (rename -> overwrite)", (done) => {
    ...
    host.rename(path`/hello`, path`/blue`).subscribe(undefined, done.fail);
    host
      .write(path`/blue`, fileBuffer`beautiful world`)
      .subscribe(undefined, done.fail);
