...
describe("find-module", () => {
  describe("findModule", () => {
    ...
    it("should throw if only routing modules were found", () => {
      host = new EmptyTree();
      host.create(
        "/foo/src/app/anything-routing.module.ts",
        "anything routing module"
      );
    ...
    it("should not find module if ext is invalid", () => {
      expect(() =>
        findModule(host, "foo/src/app/bar", "-module.ts")
      ).toThrowError(/Could not find an NgModule/);
      expect(() =>
        findModule(host, "foo/src/app/bar", "_module.ts")
      ).toThrowError(/Could not find an NgModule/);
    });
  });
  ...
  describe("findModuleFromOptions", () => {
    ...
    it("should find a module when name has underscore", () => {
      tree.create(
        "/projects/my-proj/src/feature_module/app_test.module.ts",
        ""
      );
      options.path = "/projects/my-proj/src";
      options.name = "feature_module/new_component";
      const modPath = findModuleFromOptions(tree, options);
      expect(modPath).toEqual(
        "/projects/my-proj/src/feature_module/app_test.module.ts" as Path
      );
    });
    ...
    it("should find a module when name has uppercase", () => {
      ...
      const modPath = findModuleFromOptions(tree, options);
      expect(modPath).toEqual(
        "/projects/my-proj/src/featureModule/appTest.module.ts" as Path
      );
    });
    ...
    it("should find a module if flat is true", () => {
      ...
      const modPath = findModuleFromOptions(tree, options);
      expect(modPath).toEqual(
        "/projects/my-proj/src/module/app_test.module.ts" as Path
      );
    });
    ...
    it("should find a module in a sub dir", () => {
      ...
      const modPath = findModuleFromOptions(tree, options);
      expect(modPath).toEqual(
        "/projects/my-proj/src/admin/foo.module.ts" as Path
      );
    });
  ...
  describe("buildRelativePath", () => {
    it("works", () => {
      expect(buildRelativePath("/test/module", "/test/service")).toEqual(
        "./service"
      );
      expect(buildRelativePath("/test/module", "/other/service")).toEqual(
        "../other/service"
      );
      expect(buildRelativePath("/module", "/test/service")).toEqual(
        "./test/service"
      );
      expect(buildRelativePath("/test/service", "/module")).toEqual(
        "../module"
      );
    });
  });
