...
const reUnescapedHtml = new RegExp(
  `[${Object.keys(kHtmlEscapes).join("")}]`,
  "g"
);
...
function templateWithSourceMap(
  ...
  const nodes = ast.children
    .reduce((chunk, node) => {
      ...
      return chunk.add(
        new SourceNode(node.start.line, node.start.column, sourceUrl, code)
      );
    }, preamble)
    .add(
