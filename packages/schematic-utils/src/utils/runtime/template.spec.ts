...
describe("applyPathTemplate", () => {
  ...
  it("works", () => {
    ...
    expect(_applyPathTemplate("/a/b/__c__/d", { c: "hello/world" })).toBe(
      "/a/b/hello/world/d"
    );
    expect(_applyPathTemplate("/a__c__b", { c: "hello/world" })).toBe(
      "/ahello/worldb"
    );
    expect(_applyPathTemplate("/a__c__b__d__c", { c: "1", d: "2" })).toBe(
      "/a1b2c"
    );
  });

  it("works with single _", () => {
    expect(_applyPathTemplate("/a_b_c/d__e_f__g", { e_f: 1 })).toBe(
      "/a_b_c/d1g"
    );
  });
  ...
  it("works with complex _________...", () => {
    ...
    expect(
      _applyPathTemplate("_____" + "_____" + "_____" + "___", { _: "." })
    ).toBe("...___");
  });
  ...
  it("errors out on undefined values", () => {
    expect(() => _applyPathTemplate("/a__b__c", {})).toThrow(
      new OptionIsNotDefinedException("b")
    );
  });

  it("errors out on undefined or invalid pipes", () => {
    expect(() => _applyPathTemplate("/a__b@d__c", { b: 1 })).toThrow(
      new UnknownPipeException("d")
    );
...
describe("contentTemplate", () => {
  function _applyContentTemplate(content: string, options: any): string | null {
  ...
  it("works with echo token <%= ... %>", () => {
    expect(_applyContentTemplate("a<%= value %>b", { value: 123 })).toBe(
      "a123b"
    );
  });
  ...
  it("works with for", () => {
    expect(
      _applyContentTemplate(
        "a<% for (let i = 0; i < value; i++) { %>1<% } %>b",
        {
          value: 5,
        }
      )
  ...
  it("escapes strings properly", () => {
    expect(_applyContentTemplate("a<%= value %>b", { value: `'abc'` })).toBe(
      "a'abc'b"
    );
...
describe("applyTemplateFiles", () => {
  it("works with template files exclusively", (done) => {
    ...
    callRule(applyTemplates({ a: "foo" }), observableOf(tree as Tree), context)
      .toPromise()
      .then(() => {
