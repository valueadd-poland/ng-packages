 ...
 * @return Change
 */
export function insertImport(source, fileToEdit, symbolName, fileName, isDefault = false) {
    ...
    if (relevantImports.length > 0) {
        ...
        if (importTextNodes.length === 0) {
            const fallbackPosition = findNodes(relevantImports[0], ts.SyntaxKind.CloseBraceToken)[0].getStart() ||
 ...
 */
export function findNodes(node, kind, max = Infinity) {
    if (!node || max === 0) {
        return [];
    }
    const arr = [];
...
export function findNode(node, kind, text) {
...
export function _addSymbolToNgModuleMetadata(source, ngModulePath, metadataField, expression) {
...
export function addEntryComponentToModule(source, modulePath, classifiedName) {
    return _addSymbolToNgModuleMetadata(source, modulePath, 'entryComponents', classifiedName);
}
/**
 * Determine if an import already exists.
 */
export function isImported(source, classifiedName, importPath) {
...
export function addGlobal(source, modulePath, statement) {
...
export function insert(host, modulePath, changes) {
    if (changes.length < 1) {
        return;
    }
...
export function findDescribeBlockNode(node) {
...
export function findSpecDefinition(node) {
    const describeIdentifierNode = findNode(node, ts.SyntaxKind.Identifier, 'describe');
    if (!describeIdentifierNode) {
        return null;
    }
    const parent = describeIdentifierNode.parent;
    return parent.arguments[1] || null;
}
export function findNodesInBlock(node, kind) {
    const arr = [];
    if (node.kind === kind) {
        arr.push(node);
    }
...
export function addSymbolToObject(source, objectLiteralExpression, filePath, field, expression) {
...
export function addExportAstrix(host, filePath, exportFilePath) {
    const sourceFile = readIntoSourceFile(host, filePath);
    const exportDeclarations = findNodes(sourceFile, ts.SyntaxKind.ExportDeclaration);
