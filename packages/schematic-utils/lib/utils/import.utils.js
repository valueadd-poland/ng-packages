...
function doesTextContainExport(text, name) {
    return (text.includes(`export class ${name} `) ||
...
export function findDeclarationFileByName(host, name) {
    const schematicCache = SchematicCache.getInstance();
    const declarationPaths = schematicCache.read('declarationsPathsByDeclarationName') || {};
    let hostFiles = schematicCache.read('hostFiles');
    if (declarationPaths[name]) {
        return declarationPaths[name];
    }
    if (!hostFiles) {
        hostFiles = createFilesArrayFromDir(host.getDir('.'), dirEntry => !dirEntry.path.startsWith('/node_modules') &&
            !dirEntry.path.startsWith('/.') &&
            !dirEntry.path.startsWith('/dist')).filter(f => !f.endsWith('spec.ts') && f.endsWith('.ts'));
...
function isExportedByBarrel(host, path, fileNameNoExt) {
    ...
    for (let i = 0; i < exportDeclarations.length; i++) {
        const node = findNode(exportDeclarations[i], ts.SyntaxKind.StringLiteral);
        ...
        if (host.getDir(dir.path + '/' + exportPath) &&
...
export function buildImportPath(host, from, to) {
    from = normalize(from);
    to = normalize(to);
...
export function insertCustomImport(host, filePath, symbolName, fileName) {
    insert(host, filePath, [
        insertImport(readIntoSourceFile(host, filePath), filePath, symbolName, fileName)
    ]);
}
export function insertTypeImport(host, filePath, type) {
    const sourceFile = readIntoSourceFile(host, filePath);
    const types = parseType(type);
    const importSpecifiers = findNodes(sourceFile, ts.SyntaxKind.ImportSpecifier).map(is => is.name.getText());
    types.forEach(t => {
        if (isBaseType(t)) {
            return;
        }
