 ...
 */
const flattenObjectRecursively = (
  object: any,
  prefix: string
...
const flattenObject = (object: any): Record<string, string> => {
  ...
  Object.keys(object).forEach((propertyPrefix) => {
    if (typeof object[propertyPrefix] !== "string") {
      const flattenedNesting = flattenObjectRecursively(
      ...
      flattenedObject = {
        ...flattenedObject,
        [propertyPrefix]: object[propertyPrefix],
      };
    }
  });
...
const compareTranslations = (
  ...
  Object.keys(foundTranslationKeys).forEach((fileName) => {
    ...
    foundTranslationKeys[fileName]
      .map((translation) =>
        translationKeyRegex
          ? (translation.match(translationKeyRegex) || [])[0]
          : translation
      )
      .forEach((translation) => {
    ...
    if (missingTranslationsPerFile.length) {
      missingTranslations = {
        ...missingTranslations,
        [fileName]: missingTranslationsPerFile,
      };
    }
  });
...
const findTranslationKeysInFiles = async (
  ...
  const promisesResults: FilePathTranslationKeys[] = await Promise.all(
    promises
  );
  let results = {};

  promisesResults.forEach(
    (foundTranslationKeysInFile: FilePathTranslationKeys) => {
      if (foundTranslationKeysInFile.translationKeys) {
        results = {
          ...results,
          [foundTranslationKeysInFile.filePath]:
...
export const findMissingTranslations = async (
  ...
  const allTranslations = await findTranslationKeysInFiles(
    files,
    translationsRegex
  );
